...
@WebServlet("/api/support-customer")
public class SupportCustomerServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            if (!AuthorizationUtil.hasAnyPermission(request, Permission.VIEW_SUPPORT, Permission.MANAGE_SUPPORT)) {
            ...
            if ("getStats".equals(action)) {
                ...
                // Lấy ticket theo khách hàng đang đăng nhập với phân trang, lọc, tìm kiếm, sắp xếp
                ...
                if (session == null || session.getAttribute("isLoggedIn") == null) {
                    ...
                    if (customerId != null) {
                        ...
                        System.out.println("DEBUG: DAO result is null? " + (result == null));
                        if (result != null) {
                            System.out.println("DEBUG: DAO result keys: " + result.keySet());
                            System.out.println("DEBUG: DAO result values - totalRecords=" + result.get("totalRecords") + 
                        ...
                        jsonResponse.addProperty("success", true);
                        if (result != null) {
                            jsonResponse.add("data", gson.toJsonTree(result.get("data")));
                            
                            // Đảm bảo các giá trị phân trang được thêm vào
                            Object totalRecordsObj = result.get("totalRecords");
                            Object totalPagesObj = result.get("totalPages");
                            Object currentPageObj = result.get("currentPage");
                            Object pageSizeObj = result.get("pageSize");
                            
                            if (totalRecordsObj != null) {
                                jsonResponse.addProperty("totalRecords", ((Number) totalRecordsObj).intValue());
                            } else {
                                jsonResponse.addProperty("totalRecords", 0);
                            }
                            
                            if (totalPagesObj != null) {
                                jsonResponse.addProperty("totalPages", ((Number) totalPagesObj).intValue());
                            } else {
                                jsonResponse.addProperty("totalPages", 1);
                            }
                            
                            if (currentPageObj != null) {
                                jsonResponse.addProperty("currentPage", ((Number) currentPageObj).intValue());
                            } else {
                                jsonResponse.addProperty("currentPage", page);
                            }
                            
                            if (pageSizeObj != null) {
                                jsonResponse.addProperty("pageSize", ((Number) pageSizeObj).intValue());
                            } else {
                                jsonResponse.addProperty("pageSize", pageSize);
                            }
                            
                            System.out.println("DEBUG: Response - totalRecords=" + totalRecordsObj + ", totalPages=" + totalPagesObj + ", currentPage=" + currentPageObj);
                        } else {
                            jsonResponse.add("data", gson.toJsonTree(new java.util.ArrayList<>()));
                            jsonResponse.addProperty("totalRecords", 0);
                            jsonResponse.addProperty("totalPages", 1);
                            jsonResponse.addProperty("currentPage", page);
                            jsonResponse.addProperty("pageSize", pageSize);
            ...
            if (out != null) {
                ...
                System.out.println("DEBUG doGet - Final response: " + responseStr);
